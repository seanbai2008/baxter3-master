...
# golf setup class
class pp_setup():
    "Pick and Place setup program"
    def __init__(self, arm):
        # initialise ros node
        rospy.init_node("Locate", anonymous = True)

        # load the package manifest
        roslib.load_manifest("baxter3") # reads the package manifest and sets up the python library path based on the package dependencies

        # arm ("left" or "right") and set up baxter interface
        self.limb           = arm
        self.limb_interface = baxter_interface.Limb(self.limb)

        # gripper ("left" or "right") and setup baxter interface
        self.gripper = baxter_interface.Gripper(arm)
        self.gripper.calibrate()

        # create image directory
        self.image_dir = os.getenv("HOME") + "/Project/"

        # start positions used to measure table distance
        self.x     = 0.60                        # x     = front back
        self.y     = 0.20                        # y     = left right
        self.z     = 0.0                        # z     = up down
    ...
    # move a limb in position to measure table distance
    ...
    def get_distance(self, limb):
        if limb == "left":
            dist = baxter_interface.analog_io.AnalogIO('left_hand_range').state() #use IR range sensor in hand to find distance to table
        elif limb == "right":
            dist = baxter_interface.analog_io.AnalogIO('right_hand_range').state() #use IR range sensor in hand to find distance to table
        else:
...
def main():
    limb  = "left" #set left limb as initial limb
    setup = pp_setup(limb) # create pp_setup class instance
